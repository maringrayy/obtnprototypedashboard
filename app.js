...
function sourceText(source) {
  return currentLanguage === "es" ? String(source || "").replace(/^Source:\s*/, "Fuente: ") : source;
}
...
function applyLanguage() {
  ...
  const labels = document.querySelectorAll("#routeForm > label > span");
  if (labels[0]) labels[0].textContent = t("what");
  if (labels[1]) labels[1].textContent = t("county");
  if (labels[2]) labels[2].textContent = t("when");
    ...
    <section class="principle-grid">
      ${SAMPLE_PRINCIPLES.map((group) => `
        <article class="principle-column ${group.key}">
          <img class="principle-image" src="${esc(group.image)}" alt="${esc(principleTitle(group.title))}" />
